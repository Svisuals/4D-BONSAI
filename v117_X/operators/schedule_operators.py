...
class RefreshTask3DCounts(bpy.types.Operator):
    """Recalcula el número de elementos 3D (Inputs + Outputs) para todas las tareas en la lista."""
    ...
    def execute(self, context):
        try:
            # Llama a la nueva función en el core
            core.refresh_task_3d_counts(tool.Sequence)
            self.report({'INFO'}, "Conteos de '3D' actualizados.")
            
            # Forzar un redibujado de la UI para que se vean los cambios
