...
class SelectTaskRelatedProducts(bpy.types.Operator, tool.Ifc.Operator):
    ...
    def _execute(self, context):
        print(f"🔥🔥🔥 [SELECT DEBUG] SelectTaskRelatedProducts EXECUTED!")
        ...
        try:
            ...
            if self.task_ids:
                ...
                print(f"🔥 [SELECT DEBUG] Processing {len(task_ids)} tasks: {task_ids}")
                ...
                print(f"🔥 [SELECT DEBUG] Processing individual task: {self.task}")
                task = tool.Ifc.get().by_id(self.task)
                if task:
                    tasks_to_process.append(task)
            ...
            for task in tasks_to_process:
                print(f"🔥 [SELECT DEBUG] Processing task: {getattr(task, 'Name', 'No name')} (ID: {task.id()})")
            ...
            print("🔥 [SELECT DEBUG] Previous selection cleared")

            print(f"🔥 [SELECT DEBUG] Selecting {len(all_products)} products manually...")
