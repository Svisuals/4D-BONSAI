...
def update_legend_3d_hud_constraint(context):
    ...
    Rotation and location can follow the active camera or custom targets.
    """
    print("🚀 DEBUG: update_legend_3d_hud_constraint RUNNING!")
    import bpy
    import bonsai.tool as tool
...
def update_variance_calculation(self, context):
    ...
    def do_recalc():
        try:
            ...
            # Failsafe if the operator cannot be called
            pass
...
def update_active_work_schedule_id(self, context):
    ...
    try:
        ...
        if previous_ws_id != 0:
            try:
                print(f"💾 DEBUG: Saving profiles for WS {previous_ws_id}")
        ...
        print(f"🎯 DEBUG: New previous WS set: {current_ws_id}")
...
def update_active_task_index(self, context):
    ...
    if props.should_select_3d_on_task_click:
        ...
        try:
            print("🔥 [CALLBACK DEBUG] Automatic 3D Selection running...")
            ...
            print(f"🔥 [CALLBACK DEBUG] Task '{getattr(task_ifc, 'Name', 'Unknown')}': {len(outputs)} outputs + {len(inputs)} inputs = {len(all_products)} total")
            ...
            if all_products:
                ...
                if objects_to_select:
                    print(f"🔥 [CALLBACK DEBUG] Selecting {len(objects_to_select)} 3D objects...")
...
def update_legend_hud_on_group_change(self, context):
    ...
    try:
        ...
        # When a group is activated/deactivated, it is crucial to update the
        # UI state snapshot. The "Live Color Updates" mode depends on this
        # snapshot to know which profiles to apply.
        from ..operators.operator import snapshot_all_ui_state
        snapshot_all_ui_state(context)
        # --- END OF CORRECTION ---
        ...
        # Force a viewport redraw. This is crucial for the live_color_update_handler
        # to run and apply the colors of the new group.
        refresh_hud()

        print("🔄 Legend HUD cache invalidated and viewport refreshed")

        # ========================================================================
        # Trigger Live Color Update when group changes
        # ========================================================================
        print("🚀 NEW CODE: Starting Live Color Update logic...")
        try:
            import bonsai.tool as tool
            ...
            if anim_props and hasattr(anim_props, 'enable_live_color_updates'):
                ...
                if anim_props.enable_live_color_updates:
                    # DEBUG: Check current group status
                    ...
                    # Only trigger if the user has Live Color Updates enabled
                    if hasattr(tool.Sequence, 'live_color_update_handler'):
                        print("🎨 GROUP CHANGE: Triggering Live Color Update...")

                        # DEBUG: Check which active group the handler detects
                        # Duplicate handler logic for debugging
