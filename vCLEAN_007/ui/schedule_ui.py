...
from .lists_ui import BIM_UL_tasks  # Important for using the header
...
class BIM_PT_work_schedules(Panel):
    ...
    def draw_work_schedule_ui(self, work_schedule_id: int, work_schedule: dict[str, Any]) -> None:
        ...
        if self.props.active_work_schedule_id == work_schedule_id:
            ...
            if self.props.editing_type == "WORK_SCHEDULE":
                ...
                row1.prop(self.props, "should_show_column_ui", text="Schedule Columns", toggle=True, icon="SHORTDISPLAY") # --- RESTORE BUTTONS AS THEY WERE ---
                row1.prop(self.props.filters, "show_filters", text="Filter Tasks", toggle=True, icon="FILTER") # --- RESTORE BUTTONS AS THEY WERE ---

                # --- COPY 3D AND SYNC 3D BUTTONS ---
                row_io = col.row(align=True) # --- COPY 3D AND SYNC 3D BUTTONS ---
            ...
            grid = self.layout.grid_flow(columns=2, even_columns=True)  # Show the 4 buttons for any schedule
            col1 = grid.column()
            col1.label(
        ...
        # Additional UI when editing this specific schedule
        if self.props.active_work_schedule_id == work_schedule_id:
            if self.props.editing_type == "WORK_SCHEDULE":
    ...
    def draw_filter_ui(self) -> None:  
        ...
        active_filters_box = main_box.box() # 2. Active filters panel
        row = active_filters_box.row(align=True)
        row.prop(props.filters, "logic", text="")
    
        # Presets Menu
        row.operator_menu_enum("bim.apply_lookahead_filter", "time_window", text="WLA", icon="TIME")  # Presets Menu
    ...
    def draw_editable_task_ui(self, work_schedule_id: int) -> None:
        ...
        row = self.layout.row(align=True)
        row.label(text="Task Tools")  # The call to self.draw_filter_ui() was removed from here
        

        row = self.layout.row(align=True)
        
        # 1. We split the row. The first column (left) will be for the checkbox.
        #    We use a small factor to give it limited space.
        split = row.split(factor=0.15)
        
        # 2. We place the checkbox in the left column.
        #    It is now contained and cannot be stretched beyond this 25%.
        ...
        # 3. We use the second column for the buttons on the right.
        col_derecha = split.column()
        
        # 4. We create a sub-row INSIDE the right column to align the buttons.
        ...
        sub_fila_botones.operator("bim.refresh_task_3d_counts", text="", icon="FILE_REFRESH")
    ...
    def draw_editable_task_attributes_ui(self):
        ...
        try:
            ...
            for _a in self.props.task_attributes:
                if getattr(_a, "name", "") == "PredefinedType": # --- Draw PredefinedType exactly below Priority (as in Blender 4.2.1) ---
                    _predef = _a
                    break
        ...
        try:
            ...
            if tprops.tasks and self.props.active_task_index < len(tprops.tasks):
                ...
                if not user_groups:  # Call to the central logic to synchronize at the time of drawing
        ...
        try:  # It should not break the UI if something fails
            if self.tprops.tasks and self.props.active_task_index < len(self.tprops.tasks):
                ...
                if all_groups:
                    ...
                    if selected_count > 0:
                        info_row = box.row()  # Information of selected tasks
                    ...
                    if user_groups:
                        ...
                        if current_group and current_group != "DEFAULT":
                            ...
                            row.prop(_task, "selected_colortype_in_active_group", text="") # Show profile selector if a group is selected
                ...
                if self.props.show_saved_colortypes_section:
                    ...
                    if hasattr(_task, "colortype_group_choices") and _task.colortype_group_choices:
                        ...
                        sorted_choices = sorted(_task.colortype_group_choices,
                                              key=lambda x: (x.group_name != "DEFAULT", x.group_name)) # Sort: DEFAULT first, then alphabetically
    ...
    def draw_readonly_work_schedule_ui(self, work_schedule_id):
        if self.props.active_work_schedule_id == work_schedule_id:
            ...
            if self.props.editing_type == "TASKS":
                ...
                self.layout.template_list(  # REMOVED: self.draw_filter_ui() - this might be causing the problem
                    "BIM_UL_tasks",
                    "",
                    self.tprops,
                    "tasks",
            ...
            # Add bar with 4 buttons for BASELINE when not active
