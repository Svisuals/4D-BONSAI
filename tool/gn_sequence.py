...
def apply_gn_modifiers_and_drivers(context, controller_objects):
    ...
    for i, obj in enumerate(objects_to_modify):
        ...
        # Socket_1 corresponds to "Current Frame" input
        try:
            if 'Socket_1' in modifier:
                driver = modifier.driver_add('Socket_1').driver
                driver.type = 'SCRIPTED'
                driver.expression = 'frame'  # Blender built-in frame variable
                print(f"✅ Added Current Frame driver (Socket_1) to {obj.name}")
                current_frame_added = True
            else:
                print(f"🔍 Socket_1 not found in modifier for {obj.name}")
        except Exception as e:
            print(f"❌ Error creating Current Frame driver: {e}")

        if not current_frame_added:
            print(f"⚠️ Could not add Current Frame driver to {obj.name}")
            # More detailed debug
            try:
                print(f"🔍 Modifier type: {type(modifier)}")
                print(f"🔍 Modifier available attributes: {[attr for attr in dir(modifier) if 'Socket' in attr]}")
        ...
        if controller_objects:
            ...
            # Driver para Schedule Type - Socket_2
            ...
            try:
                if 'Socket_2' in modifier:
                    ...
                    print(f"✅ Added Schedule Type driver (Socket_2) to {obj.name}")
                    schedule_type_added = True
            except Exception as e:
                print(f"❌ Error creating Schedule Type driver: {e}")

            if not schedule_type_added:
                print(f"⚠️ Could not add Schedule Type driver to {obj.name}")

            # Driver para ColorType Group - Socket_3
            colortype_group_added = False
            try:
                if 'Socket_3' in modifier:
                    driver = modifier.driver_add('Socket_3').driver
                    driver.type = 'SCRIPTED'
                    var = driver.variables.new()
                    ...
                    print(f"✅ Added ColorType Group driver (Socket_3) to {obj.name}")
                    colortype_group_added = True
            except Exception as e:
                print(f"❌ Error creating ColorType Group driver: {e}")

            if not colortype_group_added:
                print(f"⚠️ Could not add ColorType Group driver to {obj.name}")

            # SPEED SETTINGS INTEGRATION: Add Speed Multiplier driver (CRITICAL) - Socket_4
            speed_multiplier_added = False
            try:
                if 'Socket_4' in modifier:
                    ...
                    print(f"✅ Added Speed Multiplier driver (Socket_4) to {obj.name}")
                    speed_multiplier_added = True
            except Exception as e:
                print(f"❌ Error creating Speed Multiplier driver: {e}")
