    ...
    try:
        ...
        try:
            ...
            if not active_group_name:
                active_group_name = getattr(animation_props, 'ColorType_groups', None) or "DEFAULT"
        except Exception:
        ...
        print(f"🔧 DEBUG ColorType: Processing task {task.id()}, active_group: {active_group_name}")
        print(f"   🔧 Animation props ColorType_groups: {getattr(animation_props, 'ColorType_groups', 'NONE')}")

        # Check animation_group_stack in detail
        ...
        for i, item in enumerate(stack):
            ...
            if enabled and group:
                ...
                print(f"   ✅ FOUND ACTIVE GROUP: {active_group_name}")
                break
        ...
        # Get the actual ColorType object
        try:
            from bonsai.bim.module.sequence.prop import UnifiedColorTypeManager
            # Try different possible method names
            if hasattr(UnifiedColorTypeManager, 'get_colortype_by_name'):
                ColorType = UnifiedColorTypeManager.get_colortype_by_name(context, active_group_name, colortype_name)
            elif hasattr(UnifiedColorTypeManager, 'get_colortype'):
                ColorType = UnifiedColorTypeManager.get_colortype(context, active_group_name, colortype_name)
            elif hasattr(UnifiedColorTypeManager, 'get'):
                ColorType = UnifiedColorTypeManager.get(context, active_group_name, colortype_name)
            else:
                raise AttributeError("No suitable method found on UnifiedColorTypeManager")
            print(f"   ✅ ColorType object retrieved: {ColorType.name if hasattr(ColorType, 'name') else 'NO_NAME'}")
            return ColorType
        except Exception as ex:
            print(f"   ⚠️ Error getting ColorType object: {ex}")
            # Return a default ColorType object
            colortype_obj = create_default_colortype(colortype_name)
            print(f"   🔧 Created default ColorType: {colortype_obj.name}")
...
def create_material_color_system(node_tree, attr_colortype, attr_animation_state, location):
    """Create simplified color system for the Super Material"""

    # Create a single ColorRamp that works for all ColorTypes
    color_ramp = node_tree.nodes.new('ShaderNodeValToRGB')
    color_ramp.location = location

    # Configure color ramp for construction sequence
    # 0.0 = before_start (gray), 0.5 = active (green), 1.0 = after_end (light green)
    color_ramp.color_ramp.elements[0].position = 0.0
    color_ramp.color_ramp.elements[0].color = (0.8, 0.8, 0.8, 1.0)  # Before start (gray)

    color_ramp.color_ramp.elements[1].position = 0.5
    color_ramp.color_ramp.elements[1].color = (0.0, 0.8, 0.0, 1.0)  # Active (green)

    # Add third element for after_end
    color_ramp.color_ramp.elements.new(1.0)
    color_ramp.color_ramp.elements[2].color = (0.6, 0.9, 0.6, 1.0)  # After end (light green)

    # Connect animation state to color ramp
    node_tree.links.new(attr_animation_state.outputs['Fac'], color_ramp.inputs['Fac'])

    print("   🎨 Created simplified material color system")
    return color_ramp
...
def update_object_solid_color_from_attributes(obj, current_frame):
    ...
    try:
        ...
        try:
            # Try to get the ColorType object from the task cache
            context = bpy.context
            cache_key = "_task_colortype_snapshot_cache_json"
            cache_raw = context.scene.get(cache_key)
            if cache_raw:
